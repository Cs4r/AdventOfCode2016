...
object AdventOfCode4 extends App {
  ...
  val puzzleInput =
    """hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
      ...
      |aoubshwq-pibbm-kcfygvcd-740[wnucy]""".stripMargin

  val checknum : (String) => String = (input) => {
    val chunks = input.split("-")
    val encryptedName = chunks.slice(0, chunks.length-1) mkString
    val letterPerOcurrences = encryptedName.foldLeft(Map[Char, Int]() withDefaultValue 0)((map, curr) => map.updated(curr, map(curr)+1)).groupBy(_._2).map(c => (c._1,c._2.keys.toList.sorted)).toSeq.sortBy(- _._1)

    letterPerOcurrences.foldLeft("")((acc, curr) => acc + curr._2.mkString).take(5)
  }

  val roomPattern = "([a-z-]+)-([0-9]+)\\[([a-z]+)\\]".r

  val sectorId : (String =>  Int) = (input) => {
    val roomPattern(_, sector, _) = input
    sector.toInt
  }
  ...
  val realRoom : (String) => Boolean = (input) => {
    val roomPattern(_, sector, realChecksum) = input
    val expectedChecksum = checknum(input)
    realChecksum == expectedChecksum
  }

  val rooms = puzzleInput.split("\n")
  ...
  val partB = realRooms.map(r => {
    (decrypt(r), sectorId(r))
  }).find(_._1.contains("north")).map(_._2).get
